...
public class Fragment_shangpin  extends Fragment implements view.View{
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        pp.xiangqing(pid+"");
        sp = getActivity().getSharedPreferences("ser", Context.MODE_PRIVATE);
        initview();
        initdata();
        return view;
    }
...
//            xq_list.add(xq);
            ...
            selerid = gson1.getSeller().getSellerid() + "";
            final String icon = gson1.getSeller().getIcon();
            final int pid = gson1.getData().getPid();
            sp.edit().putString("selerid",selerid).commit();
            if(getActivity()==null)
                return;
